...
The main branch contains the source code for the compiled firware on branch 'bin'. Initial working version:

## marlin-2.1.2-ender-3-pro-4.2.2-bltouch-pin-27-adapter-board.bin

I could not get the firmware from the Creality website to work. Settings like Z-offset would not save and many times the SD-card would not be recognised.

After hours of searching for firmware that works, I decided to complile my own firmware. This firmware is based on Marlin 2.1.2, with the default configuration for Creality v4.x boards, and pin 27 adapter board enabled (for BLTouch). Settings save to EEPROM instead of SD-card.
...
Code will be updated in the near future with minor fixes/tweaks.
